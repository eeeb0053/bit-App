...
public class PhoneApp{
    public static void main(String[] args) {
        ...
        while(true) {
            System.out.print("메뉴: 0.Exit 1.집전화걸기 2.전화받기 3.휴대폰걸기 4.아이폰서치"
                    + " 5.삼성페이 결제 >> ");
            switch (scanner.next()) {
            case "1":
                phone = new Phone();
...
class IPhone extends CelPhone{
    ...
    @Override
    public String toString() {
        return String.format("%s 제품인 %s을 사용해서 %s중에 %s을 한다",
                company, KIND, move, search);
    }
...
class GalPhone extends IPhone{
    ...
    @Override
    public String toString() {
        return String.format("%s 제품인 %s를 사용해서 %s중에  %s 한다",
